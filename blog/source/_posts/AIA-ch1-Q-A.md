...
5. **(T/F)** If f(n) = Ω(g(n)) then $$2^{f(n)} = Ω(2^{g(n)})$$.<img src="lecture.png"  style="width:30px;display:inline;box-shadow: none !important;">
  > - **False** prove it by contradiction. e.g. f(n) = 2n, g(n) = 4n.
  when c = 0.25 f(n) >= c * g(n). but $$2^{f(n)} = 4^n, 2^{g(n)} = 16^n$$, $$2^{f(n)} <= 2^{g(n)}$$. so this statement is wrong.
